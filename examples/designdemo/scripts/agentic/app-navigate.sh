...
#   scripts/agentic/app-navigate.sh --device "Pixel 6a" "/(tabs)/try"
#   scripts/agentic/app-navigate.sh "/(tabs)/try"
#   scripts/agentic/app-navigate.sh "/(tabs)/bottom-sheet"
...
if [ -z "$ROUTE" ]; then
  ...
  echo ""
  echo "Common routes:"
  echo "  /                         Theme tab (index)"
  echo "  /(tabs)/try               Try tab"
  echo "  /(tabs)/bottom-sheet      BottomSheet tab"
  exit 1
fi
