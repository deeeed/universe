 ...
 *
 * Environment:
 *   WATCHER_PORT    Metro port (default: 7765)
...
const COMMANDS = {
  async navigate(client, args, { deviceName, platform } = {}) {
    ...
    if (!routePath) {
      throw new Error('Usage: navigate <path>  (e.g. /(tabs)/components, /(tabs)/theme)');
    }
...
Routes:
  /(tabs)/components      Components tab (default)
  /(tabs)/theme           Theme tab
  /(tabs)/try             Try tab
