...
import { LayoutAnimation, Platform, StyleProp, TextStyle, UIManager } from "react-native"
...
export const Accordion = ({ data, animationConfig, singleExpand }: AccordionProps ) => {
  const [expandedIndices, setExpandedIndices] = useState<number[]>([])


  const handleHeaderPress = useCallback((index: number) => {
    LayoutAnimation.configureNext(animationConfig || LayoutAnimation.Presets.easeInEaseOut)
    setExpandedIndices(prevIndices => {
      ...
      return isExpanded ? prevIndices.filter(i => i !== index) : [...prevIndices, index]
    })
