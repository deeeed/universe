...
echo -e "${YELLOW}Setting up Publisher CLI...${NC}"

# Clean and build the package
echo -e "${YELLOW}Building package...${NC}"
yarn build:clean

CLI_PATH="./dist/bin/publisher.js"

# Verify the build output exists
if [ ! -f "$CLI_PATH" ]; then
    echo -e "${RED}❌ Build failed - publisher.js not found${NC}"
    exit 1
fi
...
ln -sf "$(pwd)/$CLI_PATH" "$HOME/.local/bin/publisher"

# Verify the symlink
if [ ! -L "$HOME/.local/bin/publisher" ]; then
...
echo -e "${GREEN}✅ Publisher CLI installed successfully!${NC}"
echo -e "Try running: publisher --help"
