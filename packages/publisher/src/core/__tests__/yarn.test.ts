...
describe('YarnService', () => {
  ...
  describe('validateAuth', () => {
    it('should validate authentication successfully', async () => {
      ...
      expect(mockExeca).toHaveBeenCalledWith('yarn', ['npm', 'whoami', '--registry', mockConfig.registry]);
    });
    ...
    it('should throw an error if execa fails', async () => {
      ...
      await expect(service.validateAuth()).rejects.toThrow('yarn npm authentication failed: Authentication error');
    });
  });
  ...
  describe('publish', () => {
    it('should publish a package successfully', async () => {
      ...
      expect(result).toEqual({ published: true, registry: mockConfig.registry });
      expect(mockExeca).toHaveBeenCalledWith('yarn', [
        'npm', 'publish', '--registry', mockConfig.registry, '--tag', mockConfig.tag, '--access', mockConfig.access
    ...
    it('should throw an error if publishing fails', async () => {
      ...
      await expect(service.publish(mockContext)).rejects.toThrow('Failed to publish package: Publish error');
    });
  });
  ...
  describe('getLatestVersion', () => {
    it('should return the latest version successfully', async () => {
      ...
      expect(mockExeca).toHaveBeenCalledWith('yarn', ['npm', 'info', 'test-package', 'version', '--registry', mockConfig.registry, '--json']);
    });
  ...
  describe('checkWorkspaceIntegrity', () => {
    it('should return true if integrity check passes', async () => {
      ...
      expect(mockExeca).toHaveBeenCalledWith('yarn', ['install', '--check-cache']);
    });
  ...
  describe('getWorkspaceVersion', () => {
    it('should return the workspace version', async () => {
      ...
      expect(mockExeca).toHaveBeenCalledWith('yarn', ['workspaces', 'info', 'test-package', '--json']);
    });

    it('should return "0.0.0" if an error occurs', async () => {
      mockExeca.mockRejectedValueOnce(new Error('Error fetching workspace version'));
  ...
  describe('updateDependencies', () => {
    it('should update dependencies successfully', async () => {
      ...
      await expect(service.updateDependencies(mockContext, ['dep1', 'dep2'])).resolves.not.toThrow();
      expect(mockExeca).toHaveBeenCalledWith('yarn', ['up', 'dep1', 'dep2'], { cwd: mockContext.path });
    });

    it('should throw an error if updating dependencies fails', async () => {
      mockExeca.mockRejectedValueOnce(new Error('Update failed'));

      await expect(service.updateDependencies(mockContext, ['dep1', 'dep2'])).rejects.toThrow('Failed to update dependencies: Update failed');
    });
  });
