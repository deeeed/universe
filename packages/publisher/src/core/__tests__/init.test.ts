...
describe('InitService', () => {
  ...
  describe('initialize', () => {
    ...
    it('should handle no packages found', async () => {
      ...
      await expect(initService.initialize([])).rejects.toThrow('No packages found to initialize');
    });
    ...
    it('should create root config if it does not exist', async () => {
      ...
      const lastWriteFileCall = (fs.writeFile as jest.Mock).mock.calls.at(-1) as [string, string] | undefined;
