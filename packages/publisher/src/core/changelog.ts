...
export class ChangelogService {
  ...
  private async generateConventionalChangelog(
    ...
    const repoUrl = await this.getRepositoryUrl(context, packageConfig);

    // Format commits with URLs, maintaining the same format as the update command
    const formattedCommits = commits.map((commit) => {
      const commitUrl = `${repoUrl}/commit/${commit.hash}`;
      return `- ${commit.message} ([${commit.hash.substring(0, 7)}](${commitUrl}))`;
    });

    // Join all commits with newlines
