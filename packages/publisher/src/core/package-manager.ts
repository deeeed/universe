...
export interface PackageManagerService {
  ...
  publish(context: PackageContext, config?: { npm: NpmConfig }): Promise<{ published: boolean; registry: string }>;
  getLatestVersion(packageName: string, config?: { npm: NpmConfig }): Promise<string>;
